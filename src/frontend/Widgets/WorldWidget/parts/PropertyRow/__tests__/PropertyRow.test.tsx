...
describe('WHEN "PropertyRow" is mounted', () => {
  ...
  it('AND user click on it, MUST run callback function', () => {
    const component = render(<PropertyRow {...defaultProps} />);
  ...
  it('AND "showAlert" provided, MUST render attention icon', () => {
    const component = render(<PropertyRow {...defaultProps} showAlert />);

    expect(component.getByTestId('attention')).toBeDefined();
  });
});
